    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          isScrolled
            ...
            : 'bg-gradient-to-b from-black/60 via-black/25 to-transparent py-5 sm:py-6 md:py-8'
        }`}
      >
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 md:px-12 flex items-center justify-between">
          ...
          >
            <span
              className={`font-serif text-xl sm:text-2xl md:text-3xl tracking-[0.2em] uppercase font-light transition-colors duration-300 ${
                isTopHero ? 'text-white' : 'text-[#141312]'
              }`}
            >
              SOBHA
            </span>
            <span
              className={`text-[8px] sm:text-[9px] tracking-[0.35em] uppercase -mt-0.5 transition-colors duration-300 ${
                isTopHero ? 'text-[#C5A880]' : 'text-[#8C827A]'
              }`}
            >
              SRI LANKA
            </span>
          ...
          <div className="flex items-center gap-2.5 sm:gap-4">
            <button
              ...
              className={`inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2 sm:py-2.5 text-[10px] sm:text-xs uppercase tracking-[0.18em] font-medium transition-all duration-300 ${
                isTopHero
            ...
            <button
              ...
              className={`p-2 lg:hidden transition-colors focus:outline-none ${
                isTopHero ? 'text-white hover:text-[#C5A880]' : 'text-[#141312] hover:text-[#8C827A]'
              }`}
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-40 bg-[#FAF8F5] flex flex-col justify-between p-6 sm:p-8 md:p-12 lg:hidden overflow-y-auto animate-in fade-in duration-300 pt-24 sm:pt-28">
          <div>
            <p className="text-[10px] tracking-[0.3em] uppercase text-[#8C827A] mb-6">
              Navigation
            </p>
            <nav className="flex flex-col space-y-4 sm:space-y-5">
              {navLinks.map((link) => (
                <button
                  key={link.path}
                  onClick={() => handleNavClick(link.path)}
                  className={`text-left font-serif text-xl sm:text-2xl tracking-wide transition-colors ${
